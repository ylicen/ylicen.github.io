...
// 使用硬编码的API密钥
const API_KEY = 'AIzaSyCZaYTqDMXkRfFuBGhz9YWSQHmANOa-XUU';
...
**Output:** Actual image files for a smooth, colorful doodle-style GIF on a white background. Make sure every frame is different enough from the previous one.`,
      ...
      if (chunk.candidates && chunk.candidates[0].content?.parts) {
        for (const part of chunk.candidates[0].content.parts) {
          if (part.inlineData && framesContainer) {
            frameCount++;
            updateStatus(`已生成第 ${frameCount} 帧`);
    ...
    if (frameCount < 2) {
      updateStatus('生成帧画面失败，请尝试其他提示词。');
      return false;
    }

    // Update status
    updateStatus('正在创建GIF...');
...
function main() {
  ...
  generateButton.addEventListener('mousemove', addMagicSparkle);
